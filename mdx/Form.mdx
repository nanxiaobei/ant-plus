...
Ant Plus 的核心使用方式 —— 直接在 Form 表单域组件的 Props 中，传入相关 props，示例：
...
| Prop              | Description                                            |
| ----------------- | ------------------------------------------------------ |
| id                | `getFieldDecorator` 参数中的 `id`，为表单域的唯一标识  |
...
_`max` `min` 的使用方式，传入例如 `max=10`，以 `=` 分割。上文「错误提示」中的 `XXX` 会被替换为表单域的 `label`。_
