...
### 2. Form.setConfig(config)

用于设置 `placeholder` 与校验提示信息，需在项目入口文件中调用。
...
Ant Plus 提供了以下「短语」形式的校验规则，配合上文的 "2. Form.setConfig(config)"，可自定义校验提示信息。
...
_`max` `min` 的使用方式为传入例如 `max=10`，以 `=` 分割。上文「提示信息」中的 `XXX` 会被替换为表单域的 `label`。_
