    ...
    return (
      <Form data={{ name: 'Emily', sex: 2 }} onFinish={onFinish}>
        ...
        <Select
          ...
          search
        />
        <Button label="" type="primary"  htmlType="submit">提交</Button>
      </Form>
    );
  }}
</Playground>
...
| 参数     | 默认值                | 说明                                                                                                                     |
| -------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| input    | '请输入'              | Input 的 `placeholder`（`tip` 为 `'short'` `'full'` 时有效，转义：`'short'` → `"请输入"`，`'full'` → `请输入用户名`）  |
| select   | '请选择'              | Select 的 `placeholder`（`tip` 为 `'short'` `'full'` 时有效，转义：`'short'` → `'请选择'`，`'full'` → `'请选择性别'`） |
...
- `config` 占位符为 `{label}`，`validateMessages` 占位符为 `${name}`
- `config` 使用 `label` 字段，`validateMessages` 使用 `name` 字段。实际开发中，`label` 更为实用（尤其在中文世界），`name` 用于数据字段，一般情况下用户应无感知
